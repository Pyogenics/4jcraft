...
public:
	...
	// 4J Added
	uint32_t getMessagesCount(int iPad) { return (int)guiMessages[iPad].size(); }
	std::wstring getMessage(int iPad, uint32_t index) { return guiMessages[iPad].at(index).string; }
	float getOpacity(int iPad, uint32_t index);
