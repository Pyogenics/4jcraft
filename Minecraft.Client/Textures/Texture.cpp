...
void Texture::fill(const Rect2i *rect, int color)
{
	...
	for (int y = myRect->getY(); y < (myRect->getY() + myRect->getHeight()); y++)
	{
		...
		{
			data[0]->put(line + x * 4 + 0, (uint8_t)((color >> 24) & 0x000000ff));
			data[0]->put(line + x * 4 + 1, (uint8_t)((color >> 16) & 0x000000ff));
			data[0]->put(line + x * 4 + 2, (uint8_t)((color >>  8) & 0x000000ff));
			data[0]->put(line + x * 4 + 3, (uint8_t)((color >>  0) & 0x000000ff));
		}
	}
	delete myRect;
