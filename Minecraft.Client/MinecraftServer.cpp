...
#ifdef _XBOX
#include "Build/Common/XUI/XUI_DebugSetCamera.h"
#endif
...
#include "Build/Common/Telemetry/TelemetryManager.h"
...
bool MinecraftServer::initServer(int64_t seed, NetworkGameInitData *initData, uint32_t initSettings, bool findSeed)
{
	// 4J - removed
#if 0
...
//        logger.info("Done (" + (System.nanoTime() - levelNanoTime) + "ns)! For help, type \"help\" or \"?\"");
		...
		{
			delete[] (uint8_t*)initData->saveData->data;
...
bool MinecraftServer::loadLevel(LevelStorageSource *storageSource, const std::wstring& name, int64_t levelSeed, LevelType *pLevelType, NetworkGameInitData *initData)
{
...
//		m_lastSentDifficulty = pMinecraft->options->difficulty;
	...
	if( csf->doesFileExist(filepath) )
	{
		...
		ba_gameRules.data = new uint8_t[ ba_gameRules.length ];
