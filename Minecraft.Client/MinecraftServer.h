...
typedef struct _LoadSaveDataThreadParam
{
	...
	_LoadSaveDataThreadParam(LPVOID data, int64_t filesize, const std::wstring &saveName) : data( data ), fileSize( filesize ), saveName( saveName ) {}
...
private:
	...
	bool initServer(int64_t seed, NetworkGameInitData *initData, uint32_t initSettings, bool findSeed);
	void postProcessTerminate(ProgressRenderer *mcprogress);
    bool loadLevel(LevelStorageSource *storageSource, const std::wstring& name, int64_t levelSeed, LevelType *pLevelType, NetworkGameInitData *initData);
