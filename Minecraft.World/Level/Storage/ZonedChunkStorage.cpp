...
ZoneFile *ZonedChunkStorage::getZoneFile(int x, int z, bool create)
{
    ...
    if (zoneFiles.find(key) == zoneFiles.end())
	{
		...
		if ( !file.exists() )
		{
			...
			void* ch = CreateFile(wstringtofilename(file.getPath()), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
			CloseHandle(ch);
        }
...
void ZonedChunkStorage::tick()
{
    ...
    if (tickCount % (20 * 10) == 4)
	{
		...
		for( std::unordered_map<int64_t, ZoneFile *>::iterator it = zoneFiles.begin(); it != itEndZF; it++ )
		{
...
void ZonedChunkStorage::flush()
{
	...
	for( std::unordered_map<int64_t, ZoneFile *>::iterator it = zoneFiles.begin(); it != itEnd; it++ )
	{
		ZoneFile *zoneFile = it->second;
		// 4J - removed try/catch
//        try {
